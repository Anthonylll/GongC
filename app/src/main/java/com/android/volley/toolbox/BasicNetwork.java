 ...
 * {@link com.android.volley.toolbox.HttpStack}.
 */
public class BasicNetwork implements Network {
	...
	/**
	 * 如果服务端返回的header中 Cache-Control字段对应的值包含max-age 就默认使用服务端的缓存时间；
	 * 如果服务端不包含max-age，就利用本地添加的缓存时间。
	 * 
	 * @param responseHeaders
	 *            服务端返回的headers键值对
	 * @param request
	 *            客户端发出的请求
	 * @return 添加maxage的headers
	 */
	protected static Map<String, String> convertMaxAge(
			Map<String, String> responseHeaders, Request<?> request) {
		...
		if (request.shouldCache() && request.getCacheTime() != 0) {
			if (responseHeaders.containsKey("Cache-Control")) {
				...
				responseHeaders.put("Cache-Control", temp + ",max-age="
						+ request.getCacheTime());
			} else {
				responseHeaders.put("Cache-Control",
						"max-age=" + request.getCacheTime());
			}
		}
		return responseHeaders;
	}
