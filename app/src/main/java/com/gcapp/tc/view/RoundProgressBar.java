...
public class RoundProgressBar extends View {
	...
	public RoundProgressBar(Context context, AttributeSet attrs, int defStyle) {
		...
		roundProgressColor = 0xffff0012;// 红色
		textColor = mTypedArray.getColor(
				R.styleable.RoundProgressBar_textColor, Color.BLACK);

		textSize = mTypedArray.getDimension(
				R.styleable.RoundProgressBar_textSize, 20 * scale);
		roundWidth = mTypedArray.getDimension(
				R.styleable.RoundProgressBar_roundWidth, 12 * scale);
	...
	protected void onDraw(Canvas canvas) {
		...
		if (textIsDisplayable && percent != 0 && style == STROKE) {
			if (progress2 != 0) {
				...
				canvas.drawText(content1, centre
						- getTextWidth(paint, content1) / 2, centre, paint);
				paint.setColor(getResources().getColor(R.color.main_red_new));
				String content2 = "保" + progress2 + "%";
				paint.setTextSize(px2 / 7);
				canvas.drawText(content2, centre
						- getTextWidth(paint, content2) / 2, centre + px2 / 7,
						paint); // 画出进度百分比
			} else {
				paint.setTextSize(px2 / 5);
				String content3 = progress + "%";
				canvas.drawText(content3, centre
						- getTextWidth(paint, content3) / 2, centre + (px2 / 5)
						/ 2, paint);
			}
		}
		...
		case STROKE: {
			...
			// canvas.drawArc(oval, 360 * progress / max-90,360 * (100 -
			// progress) / max, false, paint);
			// }
			// else
			// canvas.drawArc(oval, 360 * progress / max-90,360 * progress2 /
			// max, false, paint); //根据进度画圆弧
			break;
		}
		case FILL: {
