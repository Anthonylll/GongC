 ...
 * 
 */
public class SelectBjdcsfPassTypePopupWindow {
	private final static String TAG = "SelectBjdcsfPassTypePopupWindow";
	...
	public SelectBjdcsfPassTypePopupWindow(Context context, int countDan,
	...
	public void init() {
		String str_show = "3串1,3串4,3串7,4串1,4串5,4串11,4串15,5串1,5串6,5串16,5串26,5串31,6串1,6串7,6串22,6串42,6串57,6串63,7串1,8串1,9串1,10串1,11串1,12串1,13串1,14串1,15串1";
		String str_passtype = "AC,AD,AE,AF,AG,AH,AI,AJ,AK,AL,AM,AN,AO,AP,AQ,AR,AS,AT,AU,AV,AW,AX,AY,AZ,BA,BB,BC";
		...
		if (0 == countDan || 1 == countDan) {// 无胆,一个胆
			for (int i = 0; i < array.size(); i++) {// 遍历array将符合条件的过关方式筛选出来
				for (int j = 2; j <= dtCount; j++) {// 筛选2串到dtCount串的过关方式
					...
					if (sb1.equals("" + j)) {
						if (pass.substring(a + 1, pass.length()).equals("1")) {// 包含1,且长度为3则为多串1
	...
	public void createPopWindow() {
		...
		btn_cancel.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {// 取消
				...
				if (null != SelectBjdcsfPassTypePopupWindow.this.listener) {
	...
	private void initViewPager() {
		...
		View view_passone = inflate.inflate(R.layout.pop_bjdc_passtype, null);

		view_passone.setTag("passone");
		listViews.add(view_passone);

		View view_passmore = inflate.inflate(R.layout.pop_bjdc_passtype, null);
