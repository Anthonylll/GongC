...
public class MyDateTimeDialog extends Dialog {
	...
	public MyDateTimeDialog(Context context, int theme,
			...
			WheelAdapter day_adapter,
			android.view.View.OnClickListener _clickLister) {
		super(context, theme);
	...
	public void setDayAdapter() {
		wheel_day.setAdapter(new NumericWheelAdapter(1, AppTools.getLastDay(y,
				m)));
	...
	public static boolean compareData(int curY, int curM, int curD, int selY,
			int selM, int selD) {
		boolean istrue = true;
		if (selY > curY) {
			istrue = false;
