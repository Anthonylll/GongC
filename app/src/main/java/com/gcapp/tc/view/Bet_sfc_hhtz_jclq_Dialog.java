...
public class Bet_sfc_hhtz_jclq_Dialog extends Dialog implements
	...
	public Bet_sfc_hhtz_jclq_Dialog(Context context, int theme, int type,
			...
			LinkedHashMap<Integer, LinkedHashMap<Integer, String>> result,int ways) {
		super(context, theme);
		// TODO Auto-generated constructor stub
		this.context = context;
		betActivity = (Bet_JCLQ_Activity) context;
	...
	public void getresult() {
		if (type == 0 && ways ==1) {
			...
			if (ExpandAdapter_jclq_52_danguan.map_hashMap_cbf.get(grouid).size() == 0)
				ExpandAdapter_jclq_52_danguan.map_hashMap_cbf.remove(grouid);

			System.out.println("=====map_hashMap_cb=="
					+ ExpandAdapter_jclq_52_danguan.map_hashMap_cbf.size());

			betActivity.updateAdapter("Bet_sfc_hhtz_jclq_Dialog");

		} else if(type == 0 && ways ==0) {
			map = ExpandAdapter_jclq_52.map_hashMap_cbf.get(grouid);
			if (map == null) {
				map = new LinkedHashMap<Integer, String>();
			}
			map.remove(index);
			...
			ExpandAdapter_jclq_52.map_hashMap_cbf.put(grouid, map);

			if (ExpandAdapter_jclq_52.map_hashMap_cbf.get(grouid).size() == 0)
				ExpandAdapter_jclq_52.map_hashMap_cbf.remove(grouid);

			System.out.println("=====map_hashMap_cb=="
					+ ExpandAdapter_jclq_52.map_hashMap_cbf.size());

			betActivity.updateAdapter("Bet_sfc_hhtz_jclq_Dialog");
		}else {// 混合投注
			map = ExpandAdapter_jclq_52.map_hashMap_hhtz.get(grouid);
			...
			ExpandAdapter_jclq_52.map_hashMap_hhtz.put(grouid, map);
			betActivity.updateAdapter("Bet_sfc_hhtz_jclq_Dialog");
		}
	}
}
