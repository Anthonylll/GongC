...
public class ScreenBjdcDtMatchPopupWindow {
	private final static String TAG = "ScreenBjdcDtMatchPopupWindow";
	...
	private List<List<DtMatchBJDC>> dtMatch_list;// 所有对阵
	private ArrayList<String> name_list;// 存储所有对阵的赛事名称
	private ArrayList<String> select_name_list = new ArrayList<String>();// 存储已选对阵的赛事名称
	private List<List<DtMatchBJDC>> screenMatch_list;// 筛选之后的所有对阵

	private LayoutInflater inflater;

	public ScreenBjdcDtMatchPopupWindow(Context context,
			List<List<DtMatchBJDC>> dtMatch_list, View relyView) {
	...
	public void init() {
		...
		if (null != AppTools.lottery) {
			for (List<DtMatchBJDC> listMatch : dtMatch_list) {
				for (DtMatchBJDC match : listMatch) {
	...
	public void createPopWindow() {
		...
		btn_cancel.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {// 取消
				...
				if (null != ScreenBjdcDtMatchPopupWindow.this.listener) {
		...
		btn_confirm.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {// 确定
				...
				if (1 > select_name_list.size()) {// 未选择赛事
					MyToast.getToast(context, "请至少选择一场赛事");
				} else {
					if (null != ScreenBjdcDtMatchPopupWindow.this.listener) {
						setScreenMatchList(select_name_list, dtMatch_list);// 根据筛选赛事筛选对阵
						listener.getResult(1, screenMatch_list,
								select_name_list);
					}
					popWindow.dismiss();
				}
	 ...
	 *            所有对阵
	 */
	public void setScreenMatchList(List<String> select_name_list,
			List<List<DtMatchBJDC>> dtMatch_list) {
		screenMatch_list = new ArrayList<List<DtMatchBJDC>>();
		for (List<DtMatchBJDC> list : dtMatch_list) {
			List<DtMatchBJDC> listM = new ArrayList<DtMatchBJDC>();
			for (DtMatchBJDC dt : list) {
	...
	class MyGridViewAdapterScreenDM extends BaseAdapter {
		...
		public View getView(final int position, View contentView, ViewGroup arg2) {
			...
			holder.tv_dm_name
					.setBackgroundResource(R.drawable.select_jczq_tv_bolder_white);// 设置背景
	...
	public interface DialogResultListener {
		...
		void getResult(int resultCode,
				List<List<DtMatchBJDC>> screenMatch_list,
