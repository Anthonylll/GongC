...
public class GridViewCJDLTAdapter extends BaseAdapter {
	private static final String TAG = "GridViewCJDLTAdapter";
	...
	// 选球界面
	private Select_DLT_Activity activity;
	...
	public GridViewCJDLTAdapter(Context context, Integer[] Numbers, int c,
		...
		activity = (Select_DLT_Activity) context;
	}
				...
				if (playType == 3901) {
					...
					if (type == 1) {
						...
						if (redSet.contains(content)) {
							...
							// 判断选球数量 不能超过20个
							if (redSet.size() >= 16) {
								MyToast.getToast(context, "最多允许选16个");
							} else {
								redSet.add(content);
							}
						}
				...
				// 如果为 胆拖 玩法
					...
					if (type == 1) {
						...
						if (redSet.contains(content)) {
							...
							// 如果选中了
							if (redSet.size() == 4) {
								MyToast.getToast(context, "最多允许选4个");
								return;
							} else {
						...
						if (redTuoSet.contains(content)) {
							...
							if (redSet.contains(content)) {
								...
								if (count > 10000) {
									redSet.add(content);
									redTuoSet.remove(content);
									Toast.makeText(context, "投注金额不能超过20000",
				...
				// 如果为 后区胆拖 玩法
					...
					if (type == 1) {
						...
						if (redSet.contains(content)) {
							...
							if (redSet.size() == 5) {
								MyToast.getToast(context, "最多允许选5个");
							} else
								redSet.add(content);
						}
					} else if (type == 3) {
						if (blueSet.contains(content)) {
							...
							if (blueSet.size() == 1) {
								MyToast.getToast(context, "最多允许选1个");
								return;
