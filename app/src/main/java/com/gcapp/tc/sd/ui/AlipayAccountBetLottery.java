 ...
 * @author echo
 */
public class AlipayAccountBetLottery implements OnScrollListener {
	private final static String TAG = "AlipayAccountWinBetLottery";
	private View view;
	 ...
	 *            ：视图
	 */
	public AlipayAccountBetLottery(Context context, View v) {
		this.view = v;
	 ...
	 *            ：日
	 */
	public AlipayAccountBetLottery(Context context, View v, int year,
			int month, int day) {
		this.view = v;
		this.context = context;
		myHandler = new MyHandler();
		...
		RequestUtil requestUtil = new RequestUtil(context, false, 0, true,
			...
			public void responseCallback(JSONObject jsonObject) {
				if (RequestUtil.DEBUG)
					Log.i(TAG, "充值账户明细请求结果：" + jsonObject);
				try {
					if (jsonObject != null
