 ...
 * 
 */
public class SelectPasstype_Gridview_JCAdapter extends BaseAdapter {
	private int pIndex;// 标记是多串一还是多串多
	private Context context;

	public SelectPasstype_Gridview_JCAdapter(Context context, int pIndex) {
		this.pIndex = pIndex;
	...
	public int getCount() {
		int size = 0;
		if (pIndex == 0) {
			size = SelectPasstype_List_JCAdapter.passOne.size();
		} else {
			size = SelectPasstype_List_JCAdapter.passMore.size();
		}
		return size;
	}
	...
	public View getView(final int position, View contentView, ViewGroup arg2) {
		if (pIndex == 0) {// 多串一
			...
			holder.tv_passone.setText(SelectPasstype_List_JCAdapter.passType
					.get(SelectPasstype_List_JCAdapter.passOne.get(position)));
			setBackGround(holder.iv_passone, R.drawable.left_gou_unselected,
					holder.tv_passone, R.drawable.right_unselected);
			if (SelectPasstype_List_JCAdapter.selectPassType
					.contains(SelectPasstype_List_JCAdapter.passOne
							.get(position))) {
					...
					.setOnClickListener(new View.OnClickListener() {
						...
						public void onClick(View v) {
							...
							if (SelectPasstype_List_JCAdapter.selectPassType
									.contains(SelectPasstype_List_JCAdapter.passOne
											.get(position))) {// 如果包含
								SelectPasstype_List_JCAdapter.selectPassType
										.remove(SelectPasstype_List_JCAdapter.passOne
												.get(position));// 移除

							} else {// 反之添加
								for (String temp : SelectPasstype_List_JCAdapter.selectPassType) {// 判断是否已经选了多串多
									if (SelectPasstype_List_JCAdapter.passMore
											.contains(temp)) {
										mark = true;
									}
								}
								if (!mark) {// 如果不包含多穿多就添加
									SelectPasstype_List_JCAdapter.selectPassType
											.add(SelectPasstype_List_JCAdapter.passOne
													.get(position));
								} else {
							...
							// 在这里计算注数
							if (null != SelectPasstype_List_JCAdapter.listener) {
								SelectPasstype_List_JCAdapter.listener
										.getResult(
												1,
												SelectPasstype_List_JCAdapter.selectPassType,
												0);
			...
			holder.tv_passmore.setText(SelectPasstype_List_JCAdapter.passType
					.get(SelectPasstype_List_JCAdapter.passMore.get(position)));
			setBackGround(holder.iv_passmore, R.drawable.left_gou_unselected,
					holder.tv_passmore, R.drawable.right_unselected);
			if (SelectPasstype_List_JCAdapter.selectPassType
					.contains(SelectPasstype_List_JCAdapter.passMore
							.get(position))) {
						...
						public void onClick(View v) {
							if (SelectPasstype_List_JCAdapter.selectPassType
									.contains(SelectPasstype_List_JCAdapter.passMore
											.get(position))) {// 如果包含
								SelectPasstype_List_JCAdapter.selectPassType
										.remove(SelectPasstype_List_JCAdapter.passMore
												.get(position));// 移除
							} else {// 反之添加
								boolean mark = false;
								for (String temp : SelectPasstype_List_JCAdapter.selectPassType) {// 判断是否已经选了多串多
									if (SelectPasstype_List_JCAdapter.passOne
											.contains(temp)) {
										mark = true;
									}
								}
								if (!mark) {// 如果不包含多穿多就添加
									SelectPasstype_List_JCAdapter.selectPassType
											.add(SelectPasstype_List_JCAdapter.passMore
													.get(position));
								} else {
									MyToast.getToast(context, "只能选择一种过关方式")
											;
