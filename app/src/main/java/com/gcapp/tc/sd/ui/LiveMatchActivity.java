...
/**
 * @author dm
 * @date 2018-3-29 上午11:10:07
 * @version 5.5.4
 * @Description 比分直播
 */
public class LiveMatchActivity extends Activity implements OnClickListener {

	private Context mContext = LiveMatchActivity.this;
	...
	private void getLiveMatch() {
		...
		requestUtil.getMatchData("1001",date);
	}
		...
		try {
			for (int i = 0; i < jsonArray.length(); i++) {
				...
				if (!bjsonObtect2.getString("st").equals("完场")
						&& stateTag.equals("0")) {
					//状态为非"完"的比赛
					liveMatch.setHostGrade(bjsonObtect2.getString("hs"));
					liveMatch.setGuestGrade(bjsonObtect2.getString("as"));
					String firsthost = bjsonObtect2.getString("hhs");
					String firsgest = bjsonObtect2.getString("ahs");
					//上半场时半场比分为""
					if(firsthost.equals("") || firsgest.equals("")) {
						liveMatch.setHostFirstGrade("0");
						liveMatch.setGuestFirstGrade("0");
					}else{
						liveMatch.setHostFirstGrade(bjsonObtect2.getString("hhs"));
						liveMatch.setGuestFirstGrade(bjsonObtect2.getString("ahs"));
					}
					...
					liveMatch.setHostTeam(bjsonObtect2.getString("hn2"));
					liveMatch.setGuestTeam(bjsonObtect2.getString("an2"));
					liveMatch.setGuestTeamImg(bjsonObtect2.getString("a_pic"));
					liveMatch.setHostTeamImg(bjsonObtect2.getString("h_pic"));
					liveMatch.setHostRanking(bjsonObtect2.getString("table_h"));
					liveMatch.setGuestRanking(bjsonObtect2.getString("table_a"));
	...
	private void initAdapter() {
		liveadapter = new LiveListAdapter(mContext,R.layout.activity_live_basket_child_item,matchlist);
