 ...
 * 
 */
public class SelectPasstype_Gridview_BJDCSFAdapter extends BaseAdapter {
	private int pIndex;// 标记是多串一还是多串多
	private Context context;

	public SelectPasstype_Gridview_BJDCSFAdapter(Context context, int pIndex) {
		this.pIndex = pIndex;
	...
	public View getView(final int position, View contentView, ViewGroup arg2) {
		if (pIndex == 0) {// 多串一
			...
			holder.tv_passone
					.setText(SelectPasstype_List_BJDCSFAdapter.passType
							.get(SelectPasstype_List_BJDCSFAdapter.passOne
									.get(position)));
			setBackGround(holder.iv_passone, R.drawable.left_gou_unselected,
					holder.tv_passone, R.drawable.right_unselected);
			if (SelectPasstype_List_BJDCSFAdapter.selectPassType
					.contains(SelectPasstype_List_BJDCSFAdapter.passOne
							.get(position))) {
					...
					.setOnClickListener(new View.OnClickListener() {
						...
						public void onClick(View v) {
							...
							if (SelectPasstype_List_BJDCSFAdapter.selectPassType
									.contains(SelectPasstype_List_BJDCSFAdapter.passOne
											.get(position))) {// 如果包含
								SelectPasstype_List_BJDCSFAdapter.selectPassType
										.remove(SelectPasstype_List_BJDCSFAdapter.passOne
												.get(position));// 移除
							} else {// 反之添加
								...
								if (!mark) {// 如果不包含多穿多就添加
									SelectPasstype_List_BJDCSFAdapter.selectPassType
											.add(SelectPasstype_List_BJDCSFAdapter.passOne
													.get(position));
								} else {
							...
							// 计算注数
							if (null != SelectPasstype_List_BJDCSFAdapter.listener) {
								SelectPasstype_List_BJDCSFAdapter.listener
										.getResult(
												1,
												SelectPasstype_List_BJDCSFAdapter.selectPassType,
												0);
					...
					.setOnClickListener(new View.OnClickListener() {
						...
						public void onClick(View v) {
							if (SelectPasstype_List_BJDCAdapter.selectPassType
								...
								if (!mark) {// 如果不包含多穿多就添加
									SelectPasstype_List_BJDCSFAdapter.selectPassType
											.add(SelectPasstype_List_BJDCSFAdapter.passMore
													.get(position));
								} else {
									MyToast.getToast(context, "只能选择一种过关方式")
											;
