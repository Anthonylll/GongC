...
public class MyFollowAdapter extends BaseAdapter {
	...
	public View getView(int position, View view, ViewGroup arg2) {
		...
		if (view == null) {
			...
			// 得到控件
			// holder.LinearLayout1 = (LinearLayout)
			// view.findViewById(R.id.follow_left);

			holder.follow_lv_iv_user_record1 = (ImageView) view
					.findViewById(R.id.follow_lv_iv_user_record1);
			holder.follow_lv_iv_user_record2 = (ImageView) view
					.findViewById(R.id.follow_lv_iv_user_record2);
			holder.follow_lv_iv_user_record3 = (ImageView) view
					.findViewById(R.id.follow_lv_iv_user_record3);
			holder.follow_lv_iv_user_record4 = (ImageView) view
					.findViewById(R.id.follow_lv_iv_user_record4);
			holder.follow_lv_tv_lotteryname = (TextView) view
					.findViewById(R.id.follow_lv_tv_lotteryname);
			holder.follow_lv_tv_username = (TextView) view
					.findViewById(R.id.follow_lv_tv_username);
			holder.follow_lv_iv_tatolmoney = (TextView) view
					.findViewById(R.id.follow_lv_iv_tatolmoney);
			holder.follow_lv_iv_eachmoney = (TextView) view
					.findViewById(R.id.follow_lv_iv_eachmoney);
			holder.follow_lv_iv_remain = (TextView) view
					.findViewById(R.id.follow_lv_iv_remain);
			holder.RoundProgressBar = (RoundProgressBar) view
		...
		setxin(holder.follow_lv_iv_user_record1,
				holder.follow_lv_iv_user_record2,
				holder.follow_lv_iv_user_record3,
				holder.follow_lv_iv_user_record4, View.GONE);
		...
		holder.follow_lv_iv_tatolmoney.setText(df.format(schemes.getMoney())
				+ "元");
		holder.follow_lv_iv_eachmoney
				.setText(df.format(schemes.getShareMoney()) + "元");
		...
		if (9999 < level) {// 1000以上
			setxin(holder.follow_lv_iv_user_record1,
					holder.follow_lv_iv_user_record2,
					holder.follow_lv_iv_user_record3,
					holder.follow_lv_iv_user_record4, View.VISIBLE);
			holder.follow_lv_iv_user_record1
			...
			holder.follow_lv_iv_user_record4
			...
			if (cup != 0) {
				...
				holder.follow_lv_iv_user_record2.setVisibility(View.GONE);
			}
			...
			if (medal != 0) {
				...
				holder.follow_lv_iv_user_record3.setVisibility(View.GONE);
			}
			...
			if (star != 0) {
				...
				holder.follow_lv_iv_user_record4.setVisibility(View.GONE);
			}
			...
			holder.follow_lv_iv_user_record2
					.setBackgroundResource(AppTools.level_cup_list.get(cup - 1));
		...
		// Log.i(TAG,"schemes.getSchedule()"+schemes.getSchedule());
		// Log.i(TAG,"(int)(schemes.getAssureMoney()/schemes.getMoney()*100)"+(int)(schemes.getAssureMoney()/schemes.getMoney()*100));
		holder.RoundProgressBar.setjidu(schemes.getSchedule(),
				(int) (schemes.getAssureMoney() / schemes.getMoney() * 100)); // 方案进度
																				// 保底进度
																				// 半径
		holder.RoundProgressBar.setProgress(schemes.getSchedule());
		// Log.i(TAG,
		// "position==="+position+"    jindu==="+schemes.getSchedule()+
		// "    保底==="+(schemes.getAssureMoney() / schemes.getMoney() * 100));
		return view;
	}
	...
	static class ViewHolder {
		...
		ImageView follow_lv_iv_user_record1, follow_lv_iv_user_record2,
