...
import com.gcapp.tc.sd.ui.adapter.MyBetLotterListSFCAdapter;
...
public class Bet_SFC_Activity extends Activity implements OnClickListener {
	private Context context = Bet_SFC_Activity.this;
...
//		btn_follow = (TextView) this.findViewById(R.id.btn_follow);
		...
		btn_playtype.setText("胜负彩投注");
		adapter = new MyBetLotterListSFCAdapter(Bet_SFC_Activity.this,
				Select_SFC_Activity.lisTeamArrays, AppTools.list_numbers);
...
//		}
	...
	private void init() {
		if (SFC_TeamArrayAdapter.btnMap != null
			...
			numbers.setMoney(AppTools.totalCount);
...
//		btn_follow.setOnClickListener(this);
	...
	class listItemClick implements OnItemClickListener {
		...
		public void onItemClick(AdapterView<?> arg0, View arg1, int position,
			...
			for (Entry<Integer, String> mIntegerStringEntry : btnMap.entrySet()) {
				Entry entry = (Entry) mIntegerStringEntry;
	...
	public void onClick(View v) {
		...
		case R.id.bet_btn_deleteall:
			if (AppTools.list_numbers.size() == 0) {
				MyToast.getToast(Bet_SFC_Activity.this, "请先选择投注内容");
				break;
			}
			dialog.show();
	...
	public void commit() {
		if (AppTools.list_numbers.size() > 0) {
			if (AppTools.user != null) {
				...
				RequestUtil requestUtil = new RequestUtil(context, false, 0,
					...
					public void responseCallback(JSONObject object) {
						...
						if ("0".equals(error)) {
							...
							LotteryUtils.showMoneyLess(Bet_SFC_Activity.this,
									totalMoney);
						} else if (error.equals("-500")) {
							MyToast.getToast(Bet_SFC_Activity.this, "连接超时")
									;
						} else {

							MyToast.getToast(Bet_SFC_Activity.this, resultMsg)
									;
						}
					}
					...
					public void responseError(VolleyError error) {
						// setEnabled(true);
						MyToast.getToast(Bet_SFC_Activity.this, "抱歉，支付出现未知错误..")
								;
						if (RequestUtil.DEBUG)
				...
				MyToast.getToast(Bet_SFC_Activity.this, "请先登陆");
				intent = new Intent(Bet_SFC_Activity.this, LoginActivity.class);
				intent.putExtra("loginType", "bet");
				Bet_SFC_Activity.this.startActivity(intent);
			}
		} else {
			MyToast.getToast(Bet_SFC_Activity.this, "请至少选择一注");
			intent = new Intent(Bet_SFC_Activity.this,
					Select_SFC_Activity.class);
			intent.putExtra("loginType", "bet");
			Bet_SFC_Activity.this.startActivity(intent);
