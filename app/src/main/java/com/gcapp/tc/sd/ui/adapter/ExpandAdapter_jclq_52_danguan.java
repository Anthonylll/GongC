...
import com.gcapp.tc.view.Select_sfc_hhtz_jclq_Dialog_danguan;
 ...
 * 
 */
public class ExpandAdapter_jclq_52_danguan extends BaseExpandableListAdapter
	...
	private LinkedHashMap<Integer, Integer> groupStatusMap;
	...
	public ExpandAdapter_jclq_52_danguan(Context context, Fragment fragment,
			List<List<DtMatch_Basketball>> listMatch, IphoneTreeView ip,
			int adapterType, int ways) {
		this.context = context;
		this.fragment = (CJLQMatchItemFragment_danguan) fragment;
		this.ip = ip;
		this.playType = adapterType;
		this.ways = ways;

		setList_Matchs(listMatch);
		groupStatusMap = new LinkedHashMap<Integer, Integer>();
	}
	...
	public void setList_Matchs(List<List<DtMatch_Basketball>> _list_Matchs) {
		...
		if (Select_JCLQ_DAN_Activity.list_Matchs != null
				&& Select_JCLQ_DAN_Activity.list_Matchs_single != null) {

			for (List<DtMatch_Basketball> dtMatch_basketballs : list_Matchs) {
				if (ways == 0) {
					Select_JCLQ_DAN_Activity.list_Matchs
							.add(dtMatch_basketballs);
				} else {
					Select_JCLQ_DAN_Activity.list_Matchs_single
							.add(dtMatch_basketballs);
				}
			}
	...
	public void setMapSelects(List<String> indexs, List<String> results) {
		...
		case 5:
			...
			for (int i = 0; i < indexs.size(); i++) {
				String str = indexs.get(i);
				System.out.println("+++++++++++++ooooindexs" + str);
				String father = str.split("-")[0];
				String child = str.split("-")[1];
	...
	public View getChildView(int groupPosition, int childPosition,
		...
		LinkedHashMap<Integer, String> hashMap = map_hashMap_sf.get(groupPosition);
		LinkedHashMap<Integer, String> hashMap2 = map_hashMap_dx.get(groupPosition);
		LinkedHashMap<Integer, String> hashMap3 = map_hashMap_cbf.get(groupPosition);
		LinkedHashMap<Integer, String> hashMap4 = map_hashMap_rfsf.get(groupPosition);
		LinkedHashMap<Integer, String> hashMap5 = map_hashMap_hhtz.get(groupPosition);
		...
		chiHolder.btn_open.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				Select_sfc_hhtz_jclq_Dialog_danguan dialog = new Select_sfc_hhtz_jclq_Dialog_danguan(
	...
	private void select_huntou(int groupId, int index, int btn_type) {
		...
		LinkedHashMap<Integer, String> map = ExpandAdapter_jclq_52_danguan.map_hashMap_hhtz
				.get(groupId);
		if (map == null) {
			map = new LinkedHashMap<Integer, String>();
		}
		...
		ExpandAdapter_jclq_52_danguan.map_hashMap_hhtz.put(groupId, map);
		fragment.update();
		fragment.onButtonPressed(playType, null);
